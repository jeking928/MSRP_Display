 ...
 * EDITED FOR MSRP MOD ZEN CART 1.5.4 - 20151006 frank18  https://isonetwork.net.au
 * Added modifications for proper MSRP display (do not display MSRP when set price >/= MSRP
 *   modded code by bigsy85 - added by KetchTech 2014 01 21 0822CST 
 */
...
function zen_get_products_msrp_price($products_id) {
	    global $db;
	      $product_check = $db->Execute("select products_msrp, products_priced_by_attribute from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
	
	// is there a products_price to add to attributes
	      $products_msrp = $product_check->fields['products_msrp'];
	
	      // do not select display only attributes and attributes_price_base_included is true
	
	
	        $the_base_price = $products_msrp;
	
	      return $the_base_price;
	  }
	  
	  function zen_get_products_price_save($products_id) {
	    global $db;
	      $product_check = $db->Execute("select products_msrp, products_price, products_priced_by_attribute from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
	...
	// is there a products_price to add to attributes
	      ...
	      // do not select display only attributes and attributes_price_base_included is true
	        ...
	        $the_base_price = $products_price-$products_msrp;
...
// show case only
  ...
  // modifications for MSRP by KetchTech, code by bigsy85 ==================
